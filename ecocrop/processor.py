...
class EcoCropProcessor:
    ...
    def generate_readable_calendar_advice(self):
        """
        Use the generated calendar and selected crop/district data
        to return a human-readable summary paragraph,
        advice headline centered, and farmer action instructions.
        """
        if not self.selected_crop_data or not self.selected_district_forecast:
            raise ValueError("Crop and district forecast data must be selected first.")

        crop_name = self.selected_crop_data.get('COMNAME', 'The crop').capitalize()
        district_name = self.selected_district_forecast.get('district', 'the district').title()

        # Generate calendar DataFrame again (reuse your method)
        calendar_df = self.generate_crop_calendar()

        # Compose summary sentences month-by-month contracted
        month_summaries = []
        for _, row in calendar_df.iterrows():
            month = row['Month']
            rain = row['Expected Rainfall (mm)']
            dry_spell = "yes" if row['Dry Spell Detected'] else "no"
            irrigation = "needed" if row['Irrigation Needed'] == "Yes" else "not needed"
            activity = row['Activity']

            sentence = (f"In {month}, expected rainfall is {rain} mm, "
                        f"dry spell detected: {dry_spell}, irrigation {irrigation}, "
                        f"activity: {activity}.")
            month_summaries.append(sentence)

        summary_text = f"Calendar for {crop_name} in {district_name} has been generated. " + " ".join(month_summaries)

        # Advice based on dry spells and irrigation needs:
        total_months = len(calendar_df)
        dry_months = calendar_df['Dry Spell Detected'].sum()
        irrigation_months = (calendar_df['Irrigation Needed'] == "Yes").sum()

        if dry_months > total_months // 2:
            general_advice = ("Advice: Dry spells are common in this period, so regular irrigation is crucial "
                              "to ensure healthy crop growth.")
            farmer_action = ("Farmers should plan for frequent irrigation, especially during months with low rainfall. "
                             "Monitor soil moisture and use water-saving irrigation methods if possible.")
        else:
            general_advice = ("Advice: Rainfall is generally sufficient for crop growth; irrigation is only necessary "
                              "in dry spells.")
            farmer_action = ("Farmers should observe rainfall patterns and irrigate only when necessary to conserve water.")

        return {
            "summary": summary_text,
            "advice_header": "ADVICE",
            "advice_text": general_advice,
